...
export default function SectionTreeItem({ section, selected, expanded, draggable, onSelect, onToggle, onToggleVisibility }: SectionTreeItemProps) {
  ...
  return (
    <div style={{ marginBottom: 2 }}>
      ...
      >
        ...
        {/* Name */}
        <span style={{ 
          flex: 1, fontSize: 13, fontWeight: selected ? 500 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          color: section.disabled ? "#6B7280" : selected ? "#fff" : "#E3E3E3",
          textDecoration: section.disabled ? "line-through" : "none"
        }}>
          {displayName}
        </span>

        {/* Actions on hover or if disabled */}
        <div style={{ opacity: hovered || section.disabled ? 1 : 0, transition: "opacity 150ms", display: "flex", alignItems: "center", gap: 8 }}>
          <div 
            onClick={(e) => { e.stopPropagation(); onToggleVisibility(section.key); }}
            style={{ 
              display: "flex", alignItems: "center", justifyContent: "center", width: 16, height: 16, cursor: "pointer",
              color: section.disabled ? "#6B7280" : "#9CA3AF"
            }}
            onMouseEnter={(e) => { e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={(e) => { e.currentTarget.style.color = section.disabled ? "#6B7280" : "#9CA3AF"; }}
          >
            {section.disabled ? (

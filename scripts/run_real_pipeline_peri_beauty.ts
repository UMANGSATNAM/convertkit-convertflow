  ...
  try {
    ...
    for (const sec of fullBlueprint.pages.index.sections) {
      ...
      if (sec.componentId === "hero-luxury-editorial-v1") {
        ...
        sec.blocks = [
          { id: "rev-1", type: "review", settings: { text: "The texture is exquisite and absorbs immediately. Within a week, my skin felt completely transformed with a luminous glow.", name: "Aanya Sharma", location: "Mumbai, IN", rating: 5, avatar: ImageAssignmentService.getBestMatchForRole(classifiedImages, ["portrait_avatar"], usedImageUrls, activeNiche) } },
          { id: "rev-2", type: "review", settings: { text: "Uncompromising quality from the glass packaging to the subtle botanical scent. Worth every single rupee for discerning skincare rituals.", name: "Devika Kapoor", location: "New Delhi, IN", rating: 5, avatar: ImageAssignmentService.getBestMatchForRole(classifiedImages, ["portrait_avatar"], usedImageUrls, activeNiche) } },
          { id: "rev-3", type: "review", settings: { text: "The finest luxury serum available in India. My complexion feels hydrated, firm, and visibly radiant throughout the entire day.", name: "Rhea Mehta", location: "Bengaluru, IN", rating: 5, avatar: ImageAssignmentService.getBestMatchForRole(classifiedImages, ["portrait_avatar"], usedImageUrls, activeNiche) } }
        ];
        console.log(`✨ Assigned 3 luxury client review blocks with strict portrait checking (using niche portrait pack if needed)`);
      }
    }

...
async function main() {
  ...
  const file = "collection.all-pdps.liquid";
  const content = fs.readFileSync(path.join(process.cwd(), "dev-theme-peri", "templates", file), "utf-8");
  ...
  console.log("Pushing collection.all-pdps.liquid to theme", themeId);
  try {

...
async function main() {
  console.log(`Starting clean registration of EXACTLY ${sections.length} sections...`);

  // Step 1: Ensure ALL existing records are ARCHIVED
  await prisma.componentRegistry.updateMany({
    data: { status: 'ARCHIVED' }
  });
  console.log('Archived all existing records.');

  // Step 2: Delete any conflicting records with these componentIds to guarantee fresh insert
  const ids = sections.map(s => s.componentId);
  await prisma.componentRegistry.deleteMany({
    where: { componentId: { in: ids } }
  });

  // Step 3: Insert each of the 37 sections
  for (const s of sections) {
    ...
    console.log(`✓ Inserted [${s.category}] ${s.componentId}`);
  }
  ...
  console.log('\n========================================');
  console.log(`Total PUBLISHED: ${published} (MUST BE EXACTLY 37)`);
  console.log(`Total ARCHIVED: ${archived}`);
  console.log('========================================\n');

  const groups = await prisma.componentRegistry.groupBy({
    by: ['category'],
    where: { status: 'PUBLISHED' },
    _count: { _all: true }
  });
  console.log('Category Counts:', groups);

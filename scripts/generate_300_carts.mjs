...
import * as templates from './cart_templates.mjs';
...
async function generateAll() {
  ...
  for (const niche of niches) {
    for (let i = 1; i <= 20; i++) {
      ...
      // e.g., Cart fashion-01
      const schemaName = 'Cart ' + niche.id + '-' + paddedIndex;
      const prefix = 'crt-' + niche.id + '-' + paddedIndex;
      ...
      const filename = 'cart-' + niche.id + '-' + paddedIndex + '.liquid';

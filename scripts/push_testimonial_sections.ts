...
async function main() {
  console.log("Reading testimonial sections...");
  ...
  // Read all testimonial-*.liquid files
  const sectionsDir = path.join(process.cwd(), "dev-theme-peri", "sections");
  const sectionFiles = fs.readdirSync(sectionsDir).filter(f => f.startsWith("testimonial-") && f.endsWith(".liquid"));
  ...
  console.log(`Uploading ${Object.keys(filesToUpload).length} testimonial files to Shopify theme ${themeId}...`);
  await upsertThemeFilesBatched(shop, themeId, filesToUpload);

  console.log("\n=======================================================");
  console.log("SUCCESS! Testimonial sections uploaded to Shopify.");

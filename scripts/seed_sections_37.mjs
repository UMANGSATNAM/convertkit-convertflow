...
const themeEngineBase = path.resolve(__dirname, '../app/data/templates/theme-engine');
...
async function main() {
  console.log(`Validating & registering ${sections.length} production sections...`);

  // 1. Validate all files and schemas
  for (const s of sections) {
    const filePath = path.join(themeEngineBase, s.liquidRel);
    ...
    if (!match) {
      throw new Error(`Missing {% schema %} in ${s.liquidRel}`);
    }
    try {
      JSON.parse(match[1].trim());
    } catch (err) {
      throw new Error(`Invalid JSON in ${s.liquidRel}: ${err.message}`);
    }
  }
  console.log(`✅ All ${sections.length} files exist and pass strict JSON schema parsing!`);

  // 2. Ensure all other legacy components remain ARCHIVED
  const initialArchived = await prisma.componentRegistry.updateMany({
    where: {
      status: { not: 'ARCHIVED' }
    },
    data: {
  ...
  console.log(`Reset ${initialArchived.count} existing active records to ARCHIVED.`);

  // 3. Upsert the 37 pristine sections
  for (const s of sections) {
    ...
    console.log(`  ✓ Published: [${s.category}] ${s.componentId}`);
  }
  ...
  console.log(`\n🎉 SUCCESS!`);
  console.log(`Active PUBLISHED sections: ${published} (Expected: 37)`);
  console.log(`ARCHIVED legacy sections: ${archived}`);

...
import * as templates from './testimonial_templates.mjs';
...
async function generateAll() {
  ...
  for (const niche of niches) {
    for (let i = 1; i <= 20; i++) {
      ...
      const schemaName = 'Testimonial ' + niche.id + '-' + paddedIndex;
      const prefix = 'tst-' + niche.id + '-' + paddedIndex;
      ...
      const filename = 'testimonial-' + niche.id + '-' + paddedIndex + '.liquid';

...
## Decision #19 — Phase A: Codebase Inventory Scope
- **Rule:** Evaluate ALL liquid sections across the repository (1,074+ total from dev-theme, components, base-theme, niche folders, and the 500-lib import) to determine which can be reused (after normalization) and which must be rebuilt from scratch. The canonical component library will be assembled exclusively via extraction and normalization of these existing sections, eliminating the need to build any layout from scratch.
- **Status:** **ACTIVE**
